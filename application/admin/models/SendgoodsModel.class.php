...
class SendgoodsModel extends Model
{
    public function count($like = null)
    {
        $sql = null;
        if ($like != null) {
            $sql = " SELECT COUNT(1) AS total  FROM goodsorder WHERE is_receipt=1 and g.onumb LIKE '%" . $like . "%'";
        } else {
            $sql = " SELECT COUNT(1) AS total FROM goodsorder WHERE is_receipt=1";
        }
    ...
    public function list($pageIndex = 1, $pageSize = 10, $like = null)
    {
        if ($like != null) {
            ...
            //"WHERE is_receipt=1 and onumb LIKE'%" . $like . "%' ORDER BY onumb DESC LIMIT " . ($pageIndex - 1) . "," . $pageSize;
            ...
            $sql = " select g.gid,g.onumb,g.GsId,g.price,g.is_receipt,h.uname ,g.ocount,g.timeorder,g.addressId from goodsorder g".
                ...
                "WHERE is_receipt=1 and g.onumb LIKE'%" . $like . "%' ORDER BY g.onumb DESC LIMIT " . ($pageIndex - 1) . "," . $pageSize;



        } else {
            //$sql = " SELECT g.gid,g.onumb,g.price,g.is_receipt,h.uname FROM goodorder g  INNER JOIN hpuser h  on g.uid=h.uid ".
            //"WHERE g.is_receipt=1 ORDER BY g.gid DESC LIMIT " . ($pageIndex - 1) . "," . $pageSize;
    ...
    {
        $sql = "DELETE FROM goodsorder WHERE is_receipt=1 and gid=?";

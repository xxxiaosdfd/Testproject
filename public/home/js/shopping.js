...
$(function($){
    ...
    // });
    //商品隔行变色
    $("#myTableProduct").find("tr:odd").css("backgroundColor","#51faff");
    //商品变色
    $("#myTableProduct").find("tr").mouseover(function(){
        $(this).css("backgroundColor","#ff412f");
    }).mouseout(function(){
        if($("#myTableProduct").find("tr").index($(this))%2==1) {//判断是否奇数行
            $(this).css("backgroundColor","#9effff");
        }else{
            $(this).css("backgroundColor","#b4f9fe");
        }
    });
    //计算总价
    function totalPrice(){
